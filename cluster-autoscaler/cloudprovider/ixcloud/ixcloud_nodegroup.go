...
// ixCloudNodeGroup implements NodeGroup interface from cluster-autoscaler/cloudprovider.
//
...
func (ng *ixCloudNodeGroup) IncreaseSize(delta int) error {
	...
	err := ng.ixCloudManager.updateNodeCount(ng.UUID, size+delta)
	if err != nil {
...
func (ng *ixCloudNodeGroup) DeleteNodes(nodes []*apiv1.Node) error {
	ng.clusterUpdateLock.Lock()
	defer ng.clusterUpdateLock.Unlock()
	...
	err := ng.ixCloudManager.deleteNodes(ng.UUID, nodeRefs, size-len(nodes))
	if err != nil {
...
// DecreaseTargetSize decreases the cluster node_count.
func (ng *ixCloudNodeGroup) DecreaseTargetSize(delta int) error {
	ng.clusterUpdateLock.Lock()
	defer ng.clusterUpdateLock.Unlock()
	...
	err := ng.ixCloudManager.updateNodeCount(ng.UUID, ng.targetSize+delta)
	if err != nil {
...
func (ng *ixCloudNodeGroup) Nodes() ([]cloudprovider.Instance, error) {
	ng.clusterUpdateLock.Lock()
	defer ng.clusterUpdateLock.Unlock()

	instances, err := ng.ixCloudManager.getNodes(ng.UUID)
	if err != nil {
...
func (ng *ixCloudNodeGroup) TemplateNodeInfo() (*schedulerframework.NodeInfo, error) {
	return nil, cloudprovider.ErrNotImplemented
}

// Exist returns if this node group exists.
// Currently, always returns true.
func (ng *ixCloudNodeGroup) Exist() bool {
	return true
}

// Create creates the node group on the cloud provider side.
func (ng *ixCloudNodeGroup) Create() (cloudprovider.NodeGroup, error) {
	return nil, cloudprovider.ErrAlreadyExist
}

// Delete deletes the node group on the cloud provider side.
func (ng *ixCloudNodeGroup) Delete() error {
	return cloudprovider.ErrNotImplemented
}

// Autoprovisioned returns if the nodegroup is autoprovisioned.
func (ng *ixCloudNodeGroup) Autoprovisioned() bool {
	return false
}

// GetOptions returns NodeGroupAutoscalingOptions that should be used for this particular
// NodeGroup. Returning a nil will result in using default options.
func (ng *ixCloudNodeGroup) GetOptions(defaults config.NodeGroupAutoscalingOptions) (*config.NodeGroupAutoscalingOptions, error) {
	return nil, cloudprovider.ErrNotImplemented
}

// MaxSize returns the maximum allowed size of the node group.
func (ng *ixCloudNodeGroup) MaxSize() int {
	return ng.maxSize
}

func (ng *ixCloudNodeGroup) MinSize() int {
	return ng.minSize
}

func (ng *ixCloudNodeGroup) TargetSize() (int, error) {
